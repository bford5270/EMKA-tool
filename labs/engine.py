...
def pick_follow_up_dict(lab_def: dict, context: dict | None) -> tuple[dict, str | None]:
    """Pick the follow_up dict (severity -> {category, next_tests, ehr_plan, patient_comm}).
...
def _matches_pattern(component_values: dict, pattern: dict) -> bool:
    """Test whether a component_values dict satisfies a match pattern.

    Pattern values can be:
      - exact string/scalar: must equal the corresponding component value
      - dict with 'min' / 'max': numeric range (inclusive low, exclusive high)

    A missing key in the pattern means 'any value' for that component.
    A None or missing component value never matches a non-None pattern entry.
    """
    for key, expected in pattern.items():
        actual = component_values.get(key)
...
def evaluate_qualitative(
    ...
    """Evaluate a qualitative lab by matching component_values against the
    interpretations table (first-match-wins).

    Returns a result dict shaped like evaluate() so render_result and the
    combined session output handle qualitative and numeric uniformly.
    """
    lab_def = rules.get("labs", {}).get(lab_id)
    if not lab_def:
        return {"lab_id": lab_id, "error": f"Unknown lab: {lab_id}"}
    if lab_def.get("kind") != "qualitative":
        return {"lab_id": lab_id, "error": f"{lab_id} is not a qualitative lab"}

    slots: dict[str, Any] = {**(context or {}), **component_values}

    for interp in lab_def.get("interpretations", []):
        if _matches_pattern(component_values, interp.get("match", {})):
            ...
            return {
                ...
                "result_label": interp.get("result", "Indeterminate"),
                "severity": severity,
                "follow_up": render_follow_up(interp, slots),
                "sources": lab_def.get("sources", []),
            }
    ...
    return {
        ...
        "kind": "qualitative",
        "components": dict(component_values),
        "result_label": "Indeterminate (no matching interpretation)",
        "severity": "Unknown",
        "follow_up": None,
        "sources": lab_def.get("sources", []),
    }
